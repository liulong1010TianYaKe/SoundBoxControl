...
- (void) show:(iToastType) type {
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(2, 0, textSize.width + kComponentPadding, textSize.height + kComponentPadding)];
    ...
    switch (orientation) {
        case UIDeviceOrientationPortrait:
        {
            ...
            } else if (theSettings.gravity == iToastMyPosition) {
                UIImage *bgImage = [UIImage imageNamed:@"tap_toast_bg"];
                UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 45, 45)];
                imageView.image = bgImage;
                [v addSubview:imageView];
//                v.frame = CGRectMake(v.frame.origin.x, v.frame.origin.y, 67, 67);
                label.textColor = [UIColor colorWithRed:236/255.0 green:100/255.0 blue:0 alpha:1.0];
                label.font = [UIFont systemFontOfSize:18.0f];
                label.shadowColor = [UIColor clearColor];
                v.backgroundColor = [UIColor clearColor];
                label.center = CGPointMake(imageView.bounds.size.width/2 + 2, imageView.bounds.size.height/2);
                point = CGPointMake(window.frame.size.width - 55, 100);
            } else {
