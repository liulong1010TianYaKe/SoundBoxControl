...
@interface KugooViewController ()

@end

@implementation KugooViewController
...
-(void)setupViews{
    ...
    UIImage *image = [UIImage imageNamed:@"icon_kugoo_yinyue"];
    ...
    label2.text = @"如未安装将跳转至酷狗音乐安装页面";
    label2.font = [UIFont systemFontOfSize:15.0f];
    [pageView addSubview:label2];
    
    UILabel *label3 = [[UILabel alloc] initWithFrame:CGRectMake(10, 165, 400, 80)];
    //    label3.center = CGPointMake(SCREEN_WIDTH/2, 195);
    label3.textAlignment = NSTextAlignmentCenter;
    label3.textColor = [UIColor colorWithRed:136/255.0 green:136/255.0 blue:136/255.0 alpha:1.0f];
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"在酷狗音乐的歌曲播放控制页面，点击以下按钮，\n  即可选择庭悦音乐设备播放酷狗音乐的内容。"];
    ...
    UIImage *image2 = [UIImage imageNamed:@"icon_dlna_kugoo"];
...
//    [button setTitleColor:[UIColor colorWithRed:64/255.0 green:63/255.0 blue:63/255.0 alpha:1.0f] forState:UIControlStateNormal];
...
-(void)open:(id)sender{
    NSURL *url = [NSURL URLWithString:@"kugouURL://"];
    
    if ([[UIApplication sharedApplication] canOpenURL:url])
        [[UIApplication sharedApplication] openURL:url];
    else{
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"itms-apps://itunes.apple.com/cn/app/ku-gou-yin-le/id472208016?mt=8"]];
    }
}
